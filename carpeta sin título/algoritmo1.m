function [solution]=algoritmo1(num_problem,n,Max_arragment,ifg,ffg,fcla,Nc_v,Nsi)
...
for i=1:Nc_v
    ...
    for j=1:Ns(i,1)
        ...
        P1=feval('evaluar1',num_problem,P0(1,:),n);
        P(a1,:)=[P0 P1];
    end
end
...
for v=1:Max_arragment
    ...
    x_worst=feval('generating_set_xworst',P(:,:),Nc_v,Ns(:,1),OC(:,1),n);
    a5=0;
    for i=1:Nc_v
        a5a=a5;
        a6=0;
    ...
    for i=1:Nc_v
        ...
        if rand <(1-ifg)
            %vsom(i,1)=0;
           % nvdep=[];
            %nvdep=feval('number_varaibles_desicion_for_phrases',vector_factor,n); % Vector of number of varaibles desicion for each phrases
            a7=[];
            MH=feval('creating_matriz_rank_harmonic',KM(:,:),n);%,fitness,row);
            A3=feval('Taking_tune_of_KM_melody',fitness(:,1),KM(:,:),row);
            for l=1:n
                %a52=[];
                j1=1;
                if rand < (1-ffg)
                    %if rand < 0.5
                        a7(1,l)=A3(1,l)+(rand*(A3(2,l)-A3(1,l)));
                    %else
                    %    a7(1,l)=MH(1,l)+(rand*(MH(2,l)-MH(1,l)));
                    %end
                else
                    if rand < 0.5
        ...
        a8=feval('evaluar1',num_problem,a7(1,:),n);
    ...
    x_best=feval('generating_set_xbest',P(:,:),Nc_v,Ns(:,1),OC(:,1),n,v,x_best);
    om(:,:)=feval('update_vecto_om',vi(:,1),x_best(:,:),Nc_v,om(:,:),vsom(:,1));
